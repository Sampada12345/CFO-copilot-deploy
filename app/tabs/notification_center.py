...
def ensure_schema():
    ...
    Hrana/Turso-safe: check PRAGMA table_info and only add missing columns, so
    a swallowed 'duplicate column' error never poisons the connection's
    transaction (which on Turso would break every following statement)."""
    with get_db(DB_PATH) as conn:
        ...
        wanted = {
            "invoices":     [("reminder_date", "TEXT")],
            "email_drafts": [("scheduled_send_date", "TEXT"),
                             ("template_used", "TEXT"),
                             ("reviewed_by", "TEXT"),
                             ("reviewed_at", "TEXT")],
        }
        ...
        for col, tbl in [("reminder_date",       "invoices"),
